...
In this example, I wanted to know something about the class of the number stored as `x`.  Instead of asking R to tell me the class, I just asked it to tell me if the class of x is "logical". There are many properties that you can access through `is`, but these properties may not be the ones you care about initially.  One of the worst offenders is `is.integer()`, which tells you whether or not a quantity has `integer` type, and not whether or not it is an integer in the mathematical sense.
...
The general rule to remember here is that the "equals" symbol is involved in the syntax for a numeric comparison, it should be the second symbol.
...
`TRUE`, so you can use the "and" function to tell when two different conditions are satisfied.You can also string together applications of "and" to create more complicated conditions.
...
When the time comes for you to take an R program and make it run more efficiently, it's good to know that there are special Boolean functions designed for this purpose, namely `&&` and `||`.  These functions are very similar to the "and" and "or" functions, but they can decrease the total number of function evaluations needed in a complicated logical expression.  On the downside, they do not work with vectors and matrices the same way as "and" and "or" do.  We won't be needing these for this course.

# Exercises.

1.  When doing numerical computations, it is common for a bit of rounding error to creep in.In fact, you should always expect that `x == y` is not going to tell you if two computed values are really equal.  A replacement forthis is `isTRUE(all.equal(x, y))`, which tells you if `x` and `y` are "close." If `x` is the number `1`, then how close does `y` need to be for this to return the value `TRUE`?
...
using numeric relations on `y`, make it so that:
...
3.  Extend the truth table example from above to handle the other three Boolean functions I introduced.  I've rewritten the code below to take advantage of the vectorized nature of these functions.  Do you find this code to be more readable?
...
4.  Really look at this example, and write brief comments to explain why you should have expected each of the outputs that you got.
