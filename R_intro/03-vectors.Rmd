...
When the larger number is on the right, the colon function produces a vector of numbers that increase by 1 each time you move to the right, until doing so would exceed the larger number. If the larger number is on the right, the colon function does the same thing, but it decreases by 1 each time you move an entry to the right.  It works this way even if the numbers you give are not integers.
...
3.  Given a positive integer `n`, how can I create a vector of all zeros with
length `n`?  Try (at least) the following two different ways: using the `:`
